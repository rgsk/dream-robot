...
DEFAULT_OUT = Path("experiments/expert_demo/videos/expert_pick_place_cube.mp4")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for seed in args.seeds:
            ...
            print(f"seed {seed}: {'PLACED' if placed else 'FAILED'} ({reason}), "
                  f"{len(episode) - args.hold_frames} steps")
        # fps is the control rate, not a taste choice -- see write_video.
