"""Record two-arm demonstrations of two_arm_lift into a dataset.

    MUJOCO_GL=glfw uv run python -m dream_robot.sims.robosuite.tasks.two_arm_lift.record \
        --episodes 20 --verify

Thin on purpose, like T1's: it builds this task's env and expert, hands them to
``core.record``, and prints what came back. Nothing in ``core`` needed a change
for two arms -- the recorder writes whatever the embodiment says, which here is
16 numbers and three cameras instead of 8 and two. That is ROADMAP rule 2 doing
its job at the dataset seam as well as in the policies.
...
from dream_robot.sims.robosuite.tasks.two_arm_lift.env import TwoArmLiftTask, TaskConfig
from dream_robot.sims.robosuite.tasks.two_arm_lift.expert import ExpertActor, ExpertPolicy

REPO_ID = "dream_robot/two_arm_lift"
DEFAULT_ROOT = Path("data/robosuite/two_arm_lift")
DEFAULT_VIDEO = Path("experiments/t6/videos/dataset_roundtrip.mp4")
